...
<body>
    ...
    <main>
        <div class="modulo-descricao">
            <p><strong>👤 Relatório de Usuários:</strong> {{ $descricao }}</p>
        </div>
        ...
        @if(isset($resumo))
            <div class="resumo-box">
                ...
                <div class="resumo-item">
                    ...
                    <div class="valor">{{ $resumo['administradores'] }}</div>
                </div>
                <div class="resumo-item">
                    <div class="label">Gestores</div>
                    <div class="valor">{{ $resumo['gestores'] }}</div>
                </div>
                <div class="resumo-item">
                    <div class="label">Técnicos</div>
                    <div class="valor">{{ $resumo['tecnicos'] }}</div>
                </div>
            </div>
        @endif
            ...
            <tbody>
                @forelse($usuarios as $user)
                    <tr>
                        ...
                        <td>{{ $user['email'] }}</td>
                        <td>
                            <span class="badge badge-{{ strtolower($user['perfil_classe'] ?? 'operador') }}">
                                {{ $user['perfil'] }}
                            </span>
                        </td>
                        <td>
                            <span class="badge badge-{{ $user['status_ativo'] ? 'administrador' : 'operador' }}" style="background-color: {{ $user['status_ativo'] ? '#16a34a' : '#dc2626' }};">
                                {{ $user['status'] }}
                            </span>
                        </td>
                        <td class="text-muted">{{ $user['data_criacao'] }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <div class="rodape-info">
            ...
            <p>Este relatório foi gerado automaticamente pelo Sistema de Gestão de Ativos.
               Inclui todos os usuários cadastrados no sistema com seus respectivos perfis e status.</p>
        </div>
    </main>
