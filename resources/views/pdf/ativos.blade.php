...
@if(isset($resumo))
    ...
    <div class="info">
        ...
        <p><strong>Valor Total:</strong> {{ $resumo['valor_total'] ?? 'N/A' }}</p>
    </div>
@endif
...
<table>
    ...
    <tbody>
        @forelse($ativos as $ativo)
            <tr>
                <td><strong>{{ $ativo['etiqueta'] }}</strong></td>
                <td>{{ $ativo['nome'] }}</td>
                <td>{{ $ativo['grupo'] }}</td>
        ...
        @empty
            <tr>
                <td colspan="7" style="text-align: center; padding:10px;">Nenhum ativo encontrado</td>
            </tr>
        @endforelse
    </tbody>
</table>
...
<div style="border-top: 1px solid #000; padding-top: 10px; margin-top: 20px; font-size: 10px;">
    ...
    <p>Inventário completo de todos os ativos cadastrados. Os preços estão em Kwanzas (Kz), moeda oficial de Angola.</p>
</div>
