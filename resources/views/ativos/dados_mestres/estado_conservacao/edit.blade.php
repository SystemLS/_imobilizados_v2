...
<div class="container mx-auto py-8 max-w-3xl">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 class="text-3xl font-bold text-gray-800">✏️ Editar Estado de Conservação</h1>
        <a href="{{ route('dados_mestres.estado_conservacao.index') }}"
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <form action="{{ route('dados_mestres.estado_conservacao.update', $estado->EstadoConservacaoId) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            {{-- Nome --}}
            <div>
                <label for="Nome" class="block text-gray-700 font-semibold mb-2">Nome do Estado</label>
                <input type="text" id="Nome" name="Nome"
                       ...
                       placeholder="Ex: Bom, Danificado, Inutilizável"
                       value="{{ old('Nome', $estado->Nome) }}" required>
            </div>
