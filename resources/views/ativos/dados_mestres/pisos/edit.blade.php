...
<div class="container mx-auto max-w-xl py-10">
    <h1 class="text-2xl font-bold mb-6 text-gray-800">✏️ Editar Piso</h1>

    <form action="{{ route('dados_mestres.pisos.update', $piso->PisoId) }}" method="POST"
        ...
        <div class="mb-4">
            ...
            <select id="EdificioId" name="EdificioId"
                ...
                @foreach ($edificios as $edificio)
                    <option value="{{ $edificio->EdificioId }}" {{ $piso->EdificioId == $edificio->EdificioId ? 'selected' : '' }}>
                        {{ $edificio->Nome }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="Nome" name="Nome" value="{{ old('Nome', $piso->Nome) }}"
                   class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500">
            @error('Nome')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        ...
        <div class="flex justify-between mt-6">
            ...
            <button type="submit"
                    class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Atualizar</button>
        </div>
    </form>
</div>
@endsection
