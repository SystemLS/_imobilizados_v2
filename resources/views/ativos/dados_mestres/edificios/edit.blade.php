...
<div class="container mx-auto py-8 max-w-3xl">
    ...
    {{-- Cabeçalho --}}
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Editar Edifício</h1>
    ...
    <form action="{{ route('dados_mestres.edificios.update', $edificio->EdificioId) }}" method="POST" class="bg-white p-6 rounded-xl shadow-lg">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="text" name="Nome" id="Nome" value="{{ old('Nome', $edificio->Nome) }}"
            ...
            <select name="ProvinciaId" id="ProvinciaId"
                ...
                @foreach($provincias as $provincia)
                    <option value="{{ $provincia->ProvinciaId }}"
                        {{ old('ProvinciaId', $edificio->ProvinciaId) == $provincia->ProvinciaId ? 'selected' : '' }}>
                        {{ $provincia->Nome }}
                    </option>
                @endforeach
            </select>
